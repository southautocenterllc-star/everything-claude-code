...
def make_gen(sl,tp1,tp2):
    ...
    return gen
df=load_h1()
r=run_gauntlet("best_0.5/1.0/1.5", make_gen(0.5,1.0,1.5), df, perm_iters=600)
print(f"n={r.n_trades} PFnet={r.pf_net} PFgross={r.pf_gross} perm_p={r.perm_p} "
      f"boot_lo={r.boot_ci_low} boot_hi={r.boot_ci_high} reg={r.n_regimes_profitable}/{r.n_regimes_total} top={r.pct_pnl_top_regime}")
